...
!function (a) {
    ...
    var b = navigator.appVersion.indexOf("Mac") > -1, c = navigator.userAgent.indexOf("MSIE") > -1, d = function () {
        ...
        var c = new j, d = new k, e = new l, g = new m, h = new n, i = {
            ...
            if (f === i.TAB && j.editable().data("tabsize"))c.tab(j.editable()); else if (d && (e && f === i.Z || f === i.Y))c.redo(j.editable()); else if (d && f === i.Z)c.undo(j.editable()); else if (d && f === i.B)c.bold(j.editable()); else if (d && f === i.I)c.italic(j.editable()); else if (d && f === i.U)c.underline(j.editable()); else if (d && e && f === i.S)c.strikethrough(j.editable()); else if (d && f === i.BACKSLACH)c.removeFormat(j.editable()); else if (d && f === i.K)c.setLinkDialog(j.editable(), function (a, b) {
                h.showLinkDialog(j.dialog(), a, b)
            }); else if (d && f === i.SLASH)h.showHelpDialog(j.dialog()); else if (d && e && f === i.L)c.justifyLeft(j.editable()); else if (d && e && f === i.E)c.justifyCenter(j.editable()); else if (d && e && f === i.R)c.justifyRight(j.editable()); else if (d && e && f === i.J)c.justifyFull(j.editable()); else if (d && e && f === i.NUM7)c.insertUnorderedList(j.editable()); else if (d && e && f === i.NUM8)c.insertOrderedList(j.editable()); else if (d && f === i.LEFTBRACKET)c.outdent(j.editable()); else if (d && f === i.RIGHTBRACKET)c.indent(j.editable()); else if (d && f === i.NUM0)c.formatBlock(j.editable(), "P"); else if (d && i.NUM1 <= f && f <= i.NUM6) {
                var k = "H" + String.fromCharCode(f);
                c.formatBlock(j.editable(), k)
            } else {
                if (!d || f !== i.ENTER)return void((f === i.BACKSPACE || f === i.ENTER || f === i.SPACE) && c.recordUndo(o(a.target).editable()));
            ...
            if (f.isControlSizing(b.target)) {
                ...
                l.on("mousemove", function (a) {
                    d = {
                        x: a.clientX - p.left,
                        y: a.clientY - (p.top - q)
                    }, c.resizeTo(d, n), g.update(i, {image: m}), e.update(j, {image: m})
                }).on("mouseup", function () {
            ...
            if (e.length > 0) {
                var f, g = e.attr("data-event"), i = e.attr("data-value"), j = o(b.target), k = j.dialog(), l = j.editable(), m = j.codeable();
                if (-1 !== a.inArray(g, ["resize", "float"])) {
                    var n = j.handle(), p = n.find(".note-control-selection");
                    f = p.data("target")
                }
                if (c[g] && (l.trigger("focus"), c[g](l, i, f)), -1 !== a.inArray(g, ["backColor", "foreColor"]))d.updateRecentColor(e[0], g, i); else if ("showLinkDialog" === g)c.setLinkDialog(l, function (a, b) {
                    h.showLinkDialog(k, a, b)
                }); else if ("showImageDialog" === g)h.showImageDialog(k, r, function (a) {
                    q(l, a)
                }); else if ("showHelpDialog" === g)h.showHelpDialog(k); else if ("fullscreen" === g) {
                    var s = j.editor();
                    s.toggleClass("fullscreen");
                    var u = j.toolbar(), v = function () {
                        var b = a(window).height() - u.outerHeight();
                        l.css("height", b)
                    }, w = s.hasClass("fullscreen");
                    w ? (l.data("orgHeight", l.css("height")), a(window).resize(v).trigger("resize")) : (l.css("height", l.data("orgHeight")), a(window).off("resize")), d.updateFullscreen(u, w)
                } else if ("codeview" === g) {
                    var s = j.editor(), u = j.toolbar();
                    s.toggleClass("codeview");
                    var x = s.hasClass("codeview");
                    x ? (m.val(l.html()), m.height(l.height()), d.disable(u), m.focus()) : (l.html(m.val()), l.height(m.height()), d.enable(u), l.focus()), d.updateCodeview(j.toolbar(), x)
                }
                t(b)
            }
        }, y = 24, z = function (b) {
            var c = a(document), d = o(b.target), e = d.editable(), f = (d.codeable(), e.offset().top - c.scrollTop()), g = function (a) {
                e.height(a.clientY - (f + y))
            }, h = function () {
    ...
    }, p = function () {
        {
            ...
            }, q = (this.createLayout = function (b, g, h, j) {
                if (!b.next().hasClass("note-editor")) {
                    var k = a('<div class="note-editor"></div>');
                    if (g > 0) {
                        a('<div class="note-statusbar"><div class="note-resizebar"><div class="note-icon-bar"></div><div class="note-icon-bar"></div><div class="note-icon-bar"></div></div></div>').prependTo(k)
                    }
                    var l = a('<div class="note-editable custom-scroll" contentEditable="true"></div>').prependTo(k);
                    g && l.height(g), h && l.data("tabsize", h), l.html(f.html(b)), l.data("NoteHistory", new i);
                    {
                        a('<textarea class="note-codeable"></textarea>').prependTo(k)
                    }
                    setTimeout(function () {
                        try {
    ...
    a.fn.extend({
        summernote: function (b) {
            if (b = a.extend({toolbar: [["style", ["style"]], ["font", ["bold", "italic", "underline", "clear"]], ["fontsize", ["fontsize"]], ["color", ["color"]], ["para", ["ul", "ol", "paragraph"]], ["height", ["height"]], ["table", ["table"]], ["insert", ["link", "picture"]], ["view", ["fullscreen", "codeview"]], ["help", ["help"]]]}, b), this.each(function (c, d) {
                    var e = a(d);
